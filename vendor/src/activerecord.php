...
abstract class Activerecord
{
    ...
    /**
     * Just a helper method used by the tags, category and author models
     *
     * @return int $id
     */
    public function get_id_from_title( $title = "" )
    {
        $return = FALSE;
    ...
    private function arrange_missing_where( $where )
    {
        ...
        for ( $i=0; $i<=count($where); $i++ ) {
            ...
            if ($column != "") {
                ...
                $this->where( $this->clean_table().'.'.$column.$opera.':'.$column );
            }
        }
